...
#### BFC即块级格式化上下文，属于定位方案中的普通流。BFC是CSS的一个特性，具有这个特性的元素可以看做是一个独立的容器，容器内部的布局不会在布局上影响到外部元素，并且具有普通容器没有的一些特性。

因为BFC属于普通流，内部的box在垂直方向上会一个接一个的放置

同一个BFC下的两个相邻的box外边距会发生重叠。**把元素放到两个BFC下就不会重叠**

BFc就是一个隔离的独立容器，容器内的子元素不会影响到外面的元素，外面的元素也不会影响到内部元素。**可以实现文字环绕效果**

BFC的区域不会与浮动元素重叠，**可以实现两栏布局**

BFC容器在计算高度时，浮动元素也会参与计算。**清除浮动**
...
```
3. 父级 div 定义 overflow:auto。 同时需要父级指定宽度
